...
void RayList::addVector() {
    ...
    if (m_rayList.size() > 0) {
        // std::cout<<"addVector: last vector size= "<< m_rayList.back().size()
        // <<std::endl;
...
void RayList::insertVector(void* location, size_t inputSize) {
    ...
    while (remainingBytes != 0) {
        // std::cout << "insertVector: remainingBytes: " << remainingBytes <<
        // std::endl;
        size_t freeBytes =
            RAY_VECTOR_SIZE -
            (m_rayList.back().size() * RAY_DOUBLE_COUNT * sizeof(double));
        // std::cout << "insertVector: freeBytes: " << freeBytes << std::endl;

        // std::cout << "insertVector: rayList.size: " << m_rayList.size() <<
        // std::endl;
        ...
        // std::cout << "insertVector: bytesToCopy: " << bytesToCopy <<
        // std::endl;
        memcpy(
            (m_rayList.back().data() + m_rayList.back().size()),
