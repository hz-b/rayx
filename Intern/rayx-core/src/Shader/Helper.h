...
RAYX_FUNC_INLINE
uint output_index(uint i, Inv<MemSpace>& inv) {
...
template <typename MemSpace>
RAYX_FUNC_INLINE
uint64_t rayId(Inv<MemSpace>& inv) { return uint64_t(inv.pushConstants.rayIdStart) + uint64_t(inv.globalInvocationId); }

template <typename MemSpace>
RAYX_FUNC_INLINE
void init(Inv<MemSpace>& inv) {
    inv.finalized = false;
...
template <typename MemSpace>
RAYX_FUNC_INLINE
void recordEvent(Ray r, double w, Inv<MemSpace>& inv) {
...
template <typename MemSpace>
RAYX_FUNC_INLINE
void recordFinalEvent(Ray r, double w, Inv<MemSpace>& inv) {
    recordEvent(r, w, inv);
    inv.finalized = true;
}
