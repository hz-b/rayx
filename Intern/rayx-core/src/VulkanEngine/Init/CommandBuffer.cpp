...
VkCommandBuffer VulkanEngine::createOneTimeCommandBuffer() {
    ...
    checkVkResult(vkAllocateCommandBuffers(m_Device, &commandBufferAllocateInfo, &cmdBuffer));

    VkCommandBufferBeginInfo beginInfo = {};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    checkVkResult(vkBeginCommandBuffer(cmdBuffer, &beginInfo));
    return cmdBuffer;
}
...
void VulkanEngine::createCommandBuffers(int commandBuffersCount) {
    ...
    */
    ...
    checkVkResult(vkAllocateCommandBuffers(m_Device, &commandBufferAllocateInfo, m_CommandBuffers.data()));
}
