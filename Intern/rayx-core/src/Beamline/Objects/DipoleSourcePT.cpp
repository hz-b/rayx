...
double getfactorCriticalEnergy() {
...
double getfactorDistribution() { return 3 * FINE_STRUCTURE_CONSTANT / (4.0 * pow(PI, 2) * ELEMENTARY_CHARGE); }

double getfactorTotalPowerDipol() {
...
DipoleSource::DipoleSource(const DesignObject& dobj) : LightSource(dobj) {
    ...
    m_photonFlux = dobj.parsePhotonFlux(); //never gets called, TODO: find use
    ...
    m_criticalEnergy = RAYX::getfactorCriticalEnergy();
    m_bandwidth = 1.0e-3;
    ...
    m_gamma = std::fabs(m_electronEnergy) * getfactorElectronEnergy();
...
std::vector<unsigned int> DipoleSource::calcBatchSize() const {
...
std::vector<Ray> DipoleSource::getRays() const {
    ...
    std::vector<unsigned int> size = calcBatchSize();
    int position = 0;
...
void DipoleSource::ThreadReturns::getRaysParallel() {
    std::mt19937 rng;
    rng.seed(time(nullptr) + threadID);
    ...
    for (unsigned int i = 0; i < counter; i++) {
        phi = ((double)rng() / std::mt19937::max() - 0.5) * dip->m_horDivergence;  // horDivergence in rad

        glm::dvec3 position = dip->getXYZPosition(phi, rng);

        en = dip->getEnergy(rng);  // Verteilung nach Schwingerfunktion

        psiandstokes = dip->getPsiandStokes(en, rng);
...
double DipoleSource::getNormalFromRange(double range, std::mt19937& rng) const {
...
#pragma omp parallel
    do {
        value = ((double)rng() / std::mt19937::max() - 0.5) * 9 * range;
        Distribution = exp(expanse * value * value);
    } while (Distribution < ((double)rng() / std::mt19937::max()));

    return value;
}

glm::dvec3 DipoleSource::getXYZPosition(double phi, std::mt19937& rng) const {
    RAYX_PROFILE_SCOPE("getxyz");

    double x1 = getNormalFromRange(m_sourceWidth, rng);
    ...
    double y = getNormalFromRange(m_sourceHeight, rng);
...
PsiAndStokes DipoleSource::getPsiandStokes(double en, std::mt19937& rng) const {
    RAYX_PROFILE_SCOPE("getPsiStokes");

    PsiAndStokes psiandstokes;

    do {
        psiandstokes.psi = ((double)rng() / m_maxTwister - 0.5) * 6 * m_verDivergence;
        psiandstokes.stokes = dipoleFold(psiandstokes.psi, en, m_verEbeamDivergence, rng);
    } while (psiandstokes.stokes[0] / m_maxIntensity < (double)rng() / m_maxTwister);
...
double DipoleSource::getEnergy(std::mt19937& rng) const {
    ...
    if (m_energySpread == 0) {
        energy = m_photonEnergy;
    } else {
        do {
            energy = m_photonEnergy + ((double)rng() / m_maxTwister - 0.5) * m_energySpread;
            flux = schwinger(energy);
        } while (flux / m_maxFlux - (double)rng() / m_maxTwister < 0);
    }
...
double DipoleSource::vDivergence(double energy, double sigv) const {
    double gamma = fabs(m_electronEnergy) * getfactorElectronEnergy();  // factorElectronEnergy
    if (gamma == 0.0 || m_criticalEnergy == 0.0) {
        return 0;
    }
    double psi = getfactorOmega() * 1.e-18 * 0.1 / gamma * pow(m_criticalEnergy * 1000.0 / energy, 0.43);
...
void DipoleSource::setMaxIntensity() {
    ...
    std::mt19937 rng;
    rng.seed(time(nullptr) + rng());

    for (int i = 1; i < 250; i++) {
        psi = psi + 0.05;
        auto S = dipoleFold(psi, m_photonEnergy, 1.0, rng);
        if (smax < (S[2] + S[3])) {
            smax = S[2] + S[3];
        } else {
            break;
...
glm::dvec4 DipoleSource::dipoleFold(double psi, double photonEnergy, double sigpsi, std::mt19937& rng) const {
    ...
    for (int i = 1; i <= ln; i++) {
        do {
            sy = ((double)rng() / m_maxTwister - 0.5) * sgyp;
            zw = trsgyp * sy * sy;
            wy = exp(zw);
        } while (wy - (double)rng() / m_maxTwister < 0);
...
void DipoleSource::calcMagneticField() {
    m_magneticFieldStrength = getfactorMagneticField() * fabs(m_electronEnergy) / m_bendingRadius;
    m_criticalEnergy = getfactorCriticalEnergy() * pow(fabs(m_electronEnergy), 3) / m_bendingRadius;

    m_totalPower = getfactorTotalPowerDipol() * 0.1 * pow(fabs(m_electronEnergy), 3) * m_magneticFieldStrength * fabs(m_horDivergence) / 1000.0;
