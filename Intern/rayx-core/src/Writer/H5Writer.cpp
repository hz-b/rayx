...
int count(const BundleHistory& hist) {
    int c = 0;
...
std::vector<double> toDoubles(const BundleHistory& hist, const Format& format, int startEventID) {
...
void writeH5(const BundleHistory& hist, const std::string& filename, const Format& format, std::vector<std::string> elementNames, int startEventID) {
...
BundleHistory fromDoubles(const std::vector<double>& doubles, const Format& format) {
...
BundleHistory raysFromH5(const std::string& filename, const Format& format, unsigned int* startEventID) {
    RAYX_PROFILE_FUNCTION_STDOUT();
    BundleHistory rays;
