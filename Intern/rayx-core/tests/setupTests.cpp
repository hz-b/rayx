...
RAYX::BundleHistory traceRML(std::string filename) {
    ...
    const auto rays = tracer->trace(beamline, Sequential::No, DEFAULT_BATCH_SIZE, numElements + 2, elementsRecordMask(numSources, numElements));
...
std::vector<RAYX::Ray> rayUiCompat(std::string filename, Sequential seq) {
    ...
    const auto numSources = beamline.numSources();
    const auto numElements = beamline.numElements();
    const auto rays = tracer->trace(beamline, seq, DEFAULT_BATCH_SIZE, beamline.numElements() + 2, elementsRecordMask(numSources, numElements));
