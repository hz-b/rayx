...
RAYX::BundleHistory traceRML(std::string filename) {
    ...
    return tracer->trace(beamline, Sequential::No, DEFAULT_BATCH_SIZE, beamline.m_DesignElements.size() + 2);
}
...
std::vector<RAYX::Ray> rayUiCompat(std::string filename, Sequential seq = Sequential::No) {
    auto beamline = loadBeamline(filename);
    BundleHistory hist = tracer->trace(beamline, seq, DEFAULT_BATCH_SIZE, beamline.m_DesignElements.size() + 2);
