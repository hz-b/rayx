...
double EnergyDistribution::selectEnergy() const {
    ...
    const auto func = [&](const auto arg) -> double {
        return arg.selectEnergy(s_rng, m_isContinuous);
    };
...
double EnergyRange::selectEnergy(std::mt19937& rng, bool continuous) const {
    if (continuous) {
        ...
        double arr[3] = {m_centerEnergy - m_energySpread / 2, m_centerEnergy,
                         m_centerEnergy + m_energySpread / 2};
