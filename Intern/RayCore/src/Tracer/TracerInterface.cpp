...
namespace RAYX
{
    TracerInterface::TracerInterface() :
        ...
        m_numRays(0)
    {
        RAYX_DEBUG(std::cout << "[TracerInterf]: Creating TracerInterface..." << std::endl);
    }
    ...
    TracerInterface::TracerInterface(int numElements, int numRays) :
        ...
        m_numRays(numRays)
    {
        RAYX_DEBUG(std::cout << "[TracerInterf]: Creating TracerInterface with " << numElements << " elements and " << numRays << " rays..." << std::endl);
    }

    TracerInterface::~TracerInterface()
    {
        RAYX_DEBUG(std::cout << "[TracerInterf]: Deleting TracerInterface..." << std::endl);
    }
    ...
    bool TracerInterface::run()
    {
        ...
        std::cout << "[TracerInterf]: Runtime: " << (float(clock() - begin_time) / CLOCKS_PER_SEC) * 1000 << " ms" << std::endl;
        ...
        if (SHORTOUTPUT)
            ...
            outputFile << "Index;Xloc;Yloc;Zloc;Weight;Xdir;Ydir;Zdir;Energy;Stokes0;Stokes1;Stokes2;Stokes3;pathLength;order;lastElement;extraParam\n";
        ...
        {
            RAYX_DEBUG(std::cout << "[TracerInterf]: (*outputRayIterator).size(): " << (*outputRayIterator).size() << std::endl);
            ...
            RAYX_DEBUG(std::cout << "[TracerInterf]: sample ray: " << doubleVec[0] << ", " << doubleVec[1] << ", " << doubleVec[2] << ", " << doubleVec[3] << ", " << doubleVec[4] << ", " << doubleVec[5] << ", " << doubleVec[6] << ", energy: " << doubleVec[7] << ", stokes 0: " << doubleVec[8] << std::endl);
        ...
        RAYX_DEBUG(std::cout << "[TracerInterf]: Runtime (incl. loading rays): " << (float(clock() - begin_time) / CLOCKS_PER_SEC) * 1000 << " ms" << std::endl);
    ...
    void TracerInterface::writeToFile(const std::vector<double>& outputRays, std::ofstream& file, int index) const
    {
        ...
        RAYX_DEBUG(std::cout << "[TracerInterf]: Writing " << outputRays.size() / 8 << " rays to file..." << std::endl);
