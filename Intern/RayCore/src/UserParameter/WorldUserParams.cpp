...
namespace RAYX
{
     ...
     * @param tangentAngle  = 0 default. only used for ellipsoid mirror because the misalignment coordinate system can differ from the element coordinate system by a rotation through this angle
    */
    ...
    glm::dvec4 WorldUserParams::calcPosition(WorldUserParams prev, glm::dvec4 prev_pos, glm::dmat4x4 prev_or)
    {
        ...
        glm::dvec4 position = prev_pos - prev_or * prev.getTangentAngleRotation() * prev_offset; // remove misalignment (possibly in different coord system -> tangentAngle) from position of previous element
        ...
        position = position + orientation * getTangentAngleRotation() * new_offset; // add misalignment (possibly in different coord system -> tangentAngle) of new element to the position      
