...
/// will look at Intern/RayCore/tests/input/<filename>.rml
RAYX::Beamline loadBeamline(std::string filename) {
    std::string beamline_file = canonicalizeRepositoryPath("Intern/RayCore/tests/input/" + filename + ".rml").string();

    return RAYX::importBeamline(beamline_file);
}

/// will write to Intern/RayCore/tests/output<filename>.csv
void writeToOutputCSV(const RAYX::Rays& rays, std::string filename) {
    std::string f = canonicalizeRepositoryPath("Intern/RayCore/tests/output/" + filename + ".csv").string();
    writeCSV(rays, f);
}
...
/// will look at Intern/RayCore/tests/input/<filename>.csv
...
std::vector<RAYX::Ray> loadCSVRayUI(std::string filename) {
    std::string file = canonicalizeRepositoryPath("Intern/RayCore/tests/input/" + filename + ".csv").string();
...
void compareAgainstCorrect(std::string filename, double tolerance) {
    ...
    std::string f = canonicalizeRepositoryPath("Intern/RayCore/tests/input/" + filename + ".correct.csv").string();
    auto b = loadCSV(f);
