...
TexturedRenderSystem::TexturedRenderSystem(Device& device, VkRenderPass renderPass, const std::vector<VkDescriptorSetLayout>& setLayouts)
    : m_Device{device} {
    createPipelineLayout(setLayouts);
    createPipeline(renderPass);
}

TexturedRenderSystem::~TexturedRenderSystem() { vkDestroyPipelineLayout(m_Device.device(), m_PipelineLayout, nullptr); }

void TexturedRenderSystem::rebuild(VkRenderPass renderPass, const std::vector<VkDescriptorSetLayout>& setLayouts) {
    vkDestroyPipelineLayout(m_Device.device(), m_PipelineLayout, nullptr);
    createPipelineLayout(setLayouts);
    createPipeline(renderPass);
}

void TexturedRenderSystem::createPipelineLayout(const std::vector<VkDescriptorSetLayout>& setLayouts) {
...
void TexturedRenderSystem::createPipeline(VkRenderPass renderPass) {
    ...
    const std::string vertexShader = RAYX::canonicalizeRepositoryPath("build/bin/textured_shader_vert.spv").string();
    const std::string fragmentShader = RAYX::canonicalizeRepositoryPath("build/bin/textured_shader_frag.spv").string();
    m_Pipeline = std::make_unique<GraphicsPipeline>(m_Device, vertexShader, fragmentShader, pipelineConfig);
}

void TexturedRenderSystem::render(FrameInfo& frameInfo, const std::vector<RenderObject>& objects) {
    ...
    for (const RenderObject& obj : objects) {
        bool isTextured = obj.getIsTextured();
