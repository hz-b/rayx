...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    // Create your Application instance with your desired settings.
    *appstate = new Application(1920, 1080, "RAYX-UI", argc, argv);
    return SDL_APP_CONTINUE;  // Signal that we want to continue running.
}

SDL_AppResult SDL_AppEvent(void* appstate, SDL_Event* event) {
    RAYX_VERB << "SDL_AppEvent: event type " << event->type;
    // Optionally, forward the event to your Application, e.g.:
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    app->run();  // Call your application's main loop processing.
    return SDL_APP_CONTINUE;
}
