...
Texture::Texture(const Device& device)
    ...
    createImage(m_extent, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT);
    createImageView();
    createSampler();
    updateFromPath(getExecutablePath() / "Assets" / "textures" / "default.png");
}
...
Texture::Texture(const Device& device, VkFormat format, VkImageUsageFlags usage, VkImageAspectFlags aspectFlags, VkExtent2D extent)
    ...
    createImage(extent, VK_IMAGE_TILING_OPTIMAL, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    createImageView();
    createSampler();
}
...
void Texture::updateFromPath(const std::filesystem::path& path) {
    ...
    updateFromData(pixels, texWidth, texHeight);
...
void Texture::resize(uint32_t width, uint32_t height) {
    ...
    vkDeviceWaitIdle(m_Device.device());
    cleanup();
    createImage(m_extent, VK_IMAGE_TILING_OPTIMAL, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    createImageView();
    createSampler();
}

void Texture::createImage(VkExtent2D extent, VkImageTiling tiling, VkMemoryPropertyFlags properties) {
