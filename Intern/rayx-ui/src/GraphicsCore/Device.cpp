...
void Device::hasGflwRequiredInstanceExtensions() const {
    ...
    for (const auto& required : requiredExtensions) {
        RAYX_VERB << "\t" << required;
        if (available.find(required) == available.end()) { RAYX_EXIT << "Required extension " << required << " not available!"; }
    }
}
...
void Device::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer,
    ...
    if (size == 0) { throw std::runtime_error("Creating empty buffers is not supported!"); }
...
void Device::createImageWithInfo(const VkImageCreateInfo& imageInfo, VkMemoryPropertyFlags properties, VkImage& image,
    ...
    if (vkCreateImage(m_Device, &imageInfo, nullptr, &image) != VK_SUCCESS) { throw std::runtime_error("failed to create image!"); }
    ...
    if (vkAllocateMemory(m_Device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) { throw std::runtime_error("failed to allocate image memory!"); }

    if (vkBindImageMemory(m_Device, image, imageMemory, 0) != VK_SUCCESS) { throw std::runtime_error("failed to bind image memory!"); }
}
