...
SwapChain::~SwapChain() {
    for (auto imageView : m_imageViews) { vkDestroyImageView(m_Device.device(), imageView, nullptr); }
    m_imageViews.clear();
...
VkResult SwapChain::submitCommandBuffers(const std::vector<VkCommandBuffer>& cmdBuffers, uint32_t imageIndex) {
    if (m_imagesInFlight[imageIndex] != VK_NULL_HANDLE) { vkWaitForFences(m_Device.device(), 1, &m_imagesInFlight[imageIndex], VK_TRUE, UINT64_MAX); }
...
void SwapChain::createSwapChain() {
    ...
    if (result != VK_SUCCESS) { throw std::runtime_error("failed to create swap chain! Error code: " + std::to_string(result)); }
