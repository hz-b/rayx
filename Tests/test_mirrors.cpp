...
TEST(PlaneMirror, testSimpleParams) {
    ...
    RAYX::PlaneMirror plM =
        RAYX::PlaneMirror("planemirror", geometricalShape, width, height,
                          position, orientation, sE);  // {1,2,3,0.01,0.02,0.03}
...
TEST(SphereMirror, testParams) {
    ...
    RAYX::SphereMirror sM = RAYX::SphereMirror(
        "spheremirror", geometricalShape, width, height, incidence, position,
...
TEST(Ellips, defaultParams) {
    ...
    RAYX::Ellipsoid eb = RAYX::Ellipsoid(
        "ellipsoid", geometricalShape, width, height, w_params.calcPosition(),
...
TEST(Ellips, testParamsCSCurvature) {
    ...
    RAYX::GeometricUserParams e_params = RAYX::GeometricUserParams(incidence,
entranceArmLength, exitArmLength);
    ...
    ASSERT_DOUBLE_EQ(e_params.getAlpha(), alpha);
    ASSERT_DOUBLE_EQ(e_params.getBeta(), beta);
    RAYX::WorldUserParams w_params = RAYX::WorldUserParams(e_params.getAlpha(),
e_params.getBeta(), degToRad(azimuthal), dist, mis);

    RAYX::Ellipsoid eb = RAYX::Ellipsoid("ellipsoid", geometricalShape, width,
height, w_params.calcPosition(), w_params.calcOrientation(), incidence,
entranceArmLength, exitArmLength, coordSys, figRot, a_11, mis, sE);
    RAYX::Ellipsoid e = RAYX::Ellipsoid("ellipsoid", geometricalShape, width,
height, incidence, azimuthal, dist, entranceArmLength, exitArmLength, coordSys,
...
which calc in old RAY is used double surfaceCenterZ0 = 2579.914108547892; double
tangentAngle = 0.3358074867515476; double halfAxisC = 1758.536013753882; double
radius = -1365.026842783936; double a33 = 0.1846692617906655; double a34 =
476.4308339088623; double a44 = -4.65661e-10; double alphaE =
0.2141981204294254; double betaE = 0.8858130939325208; std::vector<double>
correctTempMis = { 0,0,0,0,0,0 };
...
e.getTempMisalignmentParams()); EXPECT_ITERABLE_DOUBLE_EQ(std::vector<double>,
sE, e.getSlopeError());

    glm::dmat4x4 correctInMat = {0.6259888054371695, -0.76201066544902907,
-0.16576417347929398, 0, 0.77983204311399357, 0.61168318281721634,
0.13306264837006021, 0, 0, -0.21256394238094042, 0.9771471590295252, 0, 0,
2646.7431914127951, -12166.962849865024, 1}; glm::dmat4x4 correctOutMat =
{0.6259888054371695, 0.77983204311399357, 0, 0, -0.6648236542987721,
0.53366897251236345, 0.52269200918172165, 0, 0.40761197743954031,
-0.32719934643922, 0.85252159124421889, 0, 0, 0, 0, 1};
    EXPECT_ITERABLE_DOUBLE_EQ(std::vector<double>, e.getInMatrix(),
glmToVector16(correctInMat)); EXPECT_ITERABLE_DOUBLE_EQ(std::vector<double>,
e.getOutMatrix(), glmToVector16(correctOutMat));

    figRot = 1;
    RAYX::Ellipsoid e2 = RAYX::Ellipsoid("ellipsoid2", geometricalShape, width,
height, incidence, azimuthal, dist, entranceArmLength, exitArmLength, coordSys,
figRot, a_11, mis, sE, NULL, false); halfAxisC = INFINITY;
    ...
    EXPECT_ITERABLE_DOUBLE_EQ(std::vector<double>, correctTempMis,
e2.getTempMisalignmentParams()); EXPECT_ITERABLE_DOUBLE_EQ(std::vector<double>,
sE, e2.getSlopeError());

    figRot = 2;
    RAYX::Ellipsoid e3 = RAYX::Ellipsoid("ellipsoid3", geometricalShape, width,
height, incidence, azimuthal, dist, entranceArmLength, exitArmLength, coordSys,
figRot, a_11, mis, sE, NULL, false); halfAxisC = 495.0186818473859;
...
which calc in old RAY is used double surfaceCenterZ0 = 2579.914108547892; double
tangentAngle = 0.3358074867515476; double halfAxisC = 1758.536013753882; double
radius = -1365.026842783936; double alpha = 0.2141981204294254; double a33 =
0.1846692617906655; double a34 = 476.4308339088623; double a44 = -4.65661e-10;
...
sE, e.getSlopeError());
    ...
    figRot = 1;
    RAYX::Ellipsoid e2 = RAYX::Ellipsoid("ellipsoid", geometricalShape, width,
height, incidence, azimuthal, dist, entranceArmLength, exitArmLength, coordSys,
figRot, a_11, mis, sE, NULL, false); halfAxisC = INFINITY;
...
sE, e2.getSlopeError());
    ...
    figRot = 2;
    RAYX::Ellipsoid e3 = RAYX::Ellipsoid("ellipsoid", geometricalShape, width,
height, incidence, azimuthal, dist, entranceArmLength, exitArmLength, coordSys,
figRot, a_11, mis, sE, NULL, false); halfAxisC = 495.0186818473859;
