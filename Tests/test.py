...
RAY_REWORKED = p_join(path, 'output')
print(RAY_REWORKED)
...
def open_new_file(name, valid = 0):
    ...
    m_o = np.array([df['Xloc'].values, df['Yloc'].values, df['Zloc'].values, df['Xdir'].values, df['Ydir'].values, df['Zdir'].values], dtype=np.double)
...
def open_old_file(name, prefix):
    ...
    m = np.array([df[prefix+'OX'].values, df[prefix+'OY'].values, df[prefix+'OZ'].values, df[prefix+'DX'].values, df[prefix+'DY'].values, df[prefix+'DZ'].values], dtype=np.double)
    return m   
...
class Tests(unittest.TestCase):
    ...
    def test_pointSourceSoftEdge(self):
        correct = open_old_file('Point Source-RawRaysBeam_SE_s.csv', 'Point Source_')
    ...
    def test_pointSourceSoftEdgeMis(self):
        correct = open_old_file('Point Source-RawRaysBeam_SE_mis_s.csv', 'Point Source_')
    ...
    def test_pointSourceHardEdgeMis(self):
        correct = open_old_file('Point Source-RawRaysBeam_HE_mis_s.csv', 'Point Source_')
