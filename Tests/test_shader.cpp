...
TEST(opticalElements, planeMirrorDefault) {
    ...
    std::shared_ptr<RAYX::PlaneMirror> plM =
        std::make_shared<RAYX::PlaneMirror>(
            ...
            200, pos_mirror, or_mirror, zeros7);  // {1,2,3,0.01,0.02,0.03}
    ...
    // std::shared_ptr<RAYX::PlaneMirror> plM =
    // std::make_shared<RAYX::PlaneMirror>("PlaneMirrorDef", 50, 200, 10, 7.5,
    // 10000, std::vector<double>{ 0,0,0, 0,0,0 }, zeros7, nullptr, true); //
    // {1,2,3,0.01,0.02,0.03} std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, plM, true);
    ASSERT_TRUE(true);
}
...
TEST(opticalElements, planeMirrorMis) {
    ...
    testOpticalElement({plM, ip}, 20);
    // std::shared_ptr<RAYX::PlaneMirror> plM =
    // std::make_shared<RAYX::PlaneMirror>("PlaneMirrorMis", 50, 200, 10, 0,
    // 10000, std::vector<double>{ 1,2,3,0.001,0.002,0.003 }, zeros7, nullptr,
    // true); // {1,2,3,0.01,0.02,0.03} std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, plM, true);
    ASSERT_TRUE(true);
}
...
TEST(opticalElements, sphereMirror) {
    ...
    std::shared_ptr<RAYX::SphereMirror> s =
        std::make_shared<RAYX::SphereMirror>(
            ...
            50, 200, radToDeg(grazingIncidence), pos_mirror, or_mirror, 10000,
            1000, zeros7);
    ...
    // std::shared_ptr<RAYX::SphereMirror> s =
    // std::make_shared<RAYX::SphereMirror>("SphereMirrorDefault", 50, 200, 10,
    // 0.0, 10000, 10000, 1000, std::vector<double>{ 0,0,0,0,0,0 }, zeros7,
    // nullptr, true); std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, s, true);
    ASSERT_TRUE(true);
}
...
TEST(opticalElements, planeGratingDevDefault) {
    ...
    std::shared_ptr<RAYX::PlaneGrating> plG =
        std::make_shared<RAYX::PlaneGrating>(
            ...
            RAYX::Geometry::GeometricalShape::RECTANGLE, 50, 200, position,
            orientation, 100, 1000, 1, 0, std::vector<double>{0, 0, 0, 0, 0, 0},
    ...
    // std::shared_ptr<RAYX::PlaneGrating> plG2 =
    // std::make_shared<RAYX::PlaneGrating>("PlaneGratingDeviationDefault", 0,
    // 50, 200, 10, 0.0, 0.0, 10000, 100, 1000, 1, 2, 0, std::vector<double>{
    // 0,0,0,0,0,0 }, std::vector<double>{ 0,0,0,0,0,0 }, zeros7, nullptr,
    // true);
    // // dx,dy,dz, dpsi,dphi,dchi // {1,2,3,0.001,0.002,0.003}
    // std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, plG2, true);
...
TEST(opticalElements, planeGratingDevAzimuthal) {
    ...
    testOpticalElement({plG, ip}, 20);
    // std::shared_ptr<RAYX::PlaneGrating> plG =
    // std::make_shared<RAYX::PlaneGrating>("PlaneGratingDeviationAz", 0, 50,
    // 200, 10, 0.0, 7.5, 10000, 100, 1000, 1, 2, 0, std::vector<double>{
    // 0,0,0,0,0,0 }, std::vector<double>{ 0,0,0,0,0,0 }, zeros7, nullptr,
    // true);
    // // dx,dy,dz, dpsi,dphi,dchi // {1,2,3,0.001,0.002,0.003}
    // std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, plG, true);
    ASSERT_TRUE(true);
}
...
TEST(opticalElements, planeGratingDevAzMis) {
    ...
    // std::shared_ptr<RAYX::PlaneGrating> plG =
    // std::make_shared<RAYX::PlaneGrating>("PlaneGratingDeviationAzMis", 0, 50,
    // 200, 10, 0.0, 7.5, 10000, 100, 1000, 1, 2, 0, std::vector<double>{
    // 1,2,3,0.001,0.002,0.003 }, std::vector<double>{ 0,0,0,0,0,0 }, zeros7,
    // nullptr, true); // dx,dy,dz, dpsi,dphi,dchi // {1,2,3,0.001,0.002,0.003}
    // std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, plG, true);
    testOpticalElement({plG, ip}, 20);
    ASSERT_TRUE(true);
...
TEST(opticalElements, planeGratingIncAzMis) {
    ...
    std::shared_ptr<RAYX::PlaneGrating> plG =
        std::make_shared<RAYX::PlaneGrating>(
            ...
            50, 200, position, orientation, 100, 1000, 1, 0,
            std::vector<double>{0, 0, 0, 0, 0, 0},
            zeros7);  // dx,dy,dz, dpsi,dphi,dchi // {1,2,3,0.001,0.002,0.003}
    testOpticalElement({plG, ip}, 20);

    // std::shared_ptr<RAYX::PlaneGrating> plG =
    // std::make_shared<RAYX::PlaneGrating>("PlaneGratingIncAzMis", 1, 50, 200,
    // 0.0, 10, 7.5, 10000, 100, 1000, 1, 2, 0, std::vector<double>{
    // 1,2,3,0.001,0.002,0.003 }, std::vector<double>{ 0,0,0,0,0,0 }, zeros7,
    // nullptr, true); // dx,dy,dz, dpsi,dphi,dchi // {1,2,3,0.001,0.002,0.003}
    // std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, plG, true);
...
TEST(opticalElements, planeGratingDevMisVLS) {
    ...
    std::shared_ptr<RAYX::PlaneGrating> plG =
        std::make_shared<RAYX::PlaneGrating>(
            ...
            RAYX::Geometry::GeometricalShape::RECTANGLE, 50, 200, position,
            orientation, 100, 1000, 1, 0,
            std::vector<double>{0.45, 0.3, 0.5, 0.2, 0.4, 0.35},
            zeros7);  // dx,dy,dz, dpsi,dphi,dchi // {1,2,3,0.001,0.002,0.003}
    ...
    // std::shared_ptr<RAYX::PlaneGrating> plG =
    // std::make_shared<RAYX::PlaneGrating>("PlaneGratingDeviationMis", 0, 50,
    // 200, 10, 0.0, 7.5, 10000, 100, 1000, 1, 2, 0, std::vector<double>{
    // 1,2,3,0.001,0.002,0.003 }, std::vector<double>{ 0.45, 0.3, 0.5, 0.2, 0.4,
    // 0.35 }, zeros7, nullptr, true); // dx,dy,dz, dpsi,dphi,dchi //
...
TEST(opticalElements, RZPDefaultParams) {
    ...
    // std::shared_ptr<RAYX::ReflectionZonePlate> rzp =
    // std::make_shared<RAYX::ReflectionZonePlate>("ReflectionZonePlateDefault",
...
TEST(opticalElements, RZPAzimuthal200) {
    ...
    // ReflectionZonePlate(const char* name, const int geometricShape, const int
    // mount, const int curvatureType, const int designType, const int
    // elementOffsetType, const double width, const double height, const double
    // deviation, const double incidenceAngle, const double azimuthal, const
    // double distanceToPreceedingElement, const double designEnergy, const
    // double sourceEnergy, const double orderOfDiffraction, const double
    // designOrderOfDiffraction, const double dAlpha, const double dBeta, const
    // double mEntrance, const double mExit, const double sEntrance, const
    // double sExit, const double shortRadius, const double longRadius, const
    // int additional_zero_order, const double elementOffsetZ, const double
    // fresnelZOffset, const double beta, const std::vector<double>
    // misalignmentParams, const std::vector<double> slopeError,

    // GeometricUserParams(int mount, int imageType, double mount, double
    // deviation, double grazingIncidence, double exitANgle, double
    // sourceWavelength, double designWavelength, double orderOfDiffraction,
    // double designOrderOfDiffraction,
    // double designAlphaAngle, double designBetaAngle, double mEntrance, double
    // mExit, double sEntrance, double sExit)

    testOpticalElement({rzp, ip}, 200);
    // std::shared_ptr<RAYX::ReflectionZonePlate> rzp =
    // std::make_shared<RAYX::ReflectionZonePlate>("ReflectionZonePlateAzim200",
    // 0, 1, 0, 0, 0, 50, 200, 170, 1, 10, 10000, 100, 100, -1, -1, 1, 1, 100,
    // 500, 100, 500, 0, 0, 0, 0, 0, 0, std::vector<double>{ 0,0,0, 0,0,0 },
    // zeros7, nullptr, true); // dx,dy,dz, dpsi,dphi,dchi //
    // {1,2,3,0.001,0.002,0.003} std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, rzp, true);
...
TEST(opticalElements, RZPMis) {
    ...
    // std::shared_ptr<RAYX::ReflectionZonePlate> rzp =
    // std::make_shared<RAYX::ReflectionZonePlate>("ReflectionZonePlateMis", 0,
    // 1, 0, 0, 0, 50, 200, 170, 1, 0, 10000, 100, 100, -1, -1, 1, 1, 100, 500,
    // 100, 500, 0, 0, 0, 0, 0, 0, std::vector<double>{ 1,2,3,0.001,0.002,0.003
    // }, zeros7, nullptr, true); // dx,dy,dz, dpsi,dphi,dchi //
    // std::shared_ptr<RAYX::ImagePlane> i =
    // std::make_shared<RAYX::ImagePlane>("ImagePlane", 1000, rzp, true);
    ASSERT_TRUE(true);
...
TEST(opticalElements, Ellipsoid_default) {
    ...
    std::shared_ptr<RAYX::Ellipsoid> eb = std::make_shared<RAYX::Ellipsoid>(
        ...
        200, pos, or1, 10, 10000, 1000, 0, 1, zeros7);
...
TEST(opticalElements, Ellipsoid_mirrormisalignment) {
    ...
    std::shared_ptr<RAYX::Ellipsoid> eb = std::make_shared<RAYX::Ellipsoid>(
        ...
        50, 200, pos, or1, 10, 10000, 1000, 0, 1, zeros7);
    eb->setOutMatrix(glmToVector16(glm::transpose(
        w_coord.calcE2B())));  // to make comparison with old ray files
                               // possible, use the beam coordinate system
...
TEST(opticalElements, EllipsoidImagePlane_ellipsmisalignment) {
    ...
    std::shared_ptr<RAYX::Ellipsoid> eb = std::make_shared<RAYX::Ellipsoid>(
        ...
        RAYX::Geometry::GeometricalShape::RECTANGLE, 50, 200, pos, or1, 10,
        10000, 1000, 0, 1, zeros7);
...
TEST(opticalElements, PlaneMirrorEllipsoidImagePlane_mirrormisalignment) {
    ...
    std::shared_ptr<RAYX::Ellipsoid> eb = std::make_shared<RAYX::Ellipsoid>(
        "ellipsoid_ip_200mirrormis",
        RAYX::Geometry::GeometricalShape::RECTANGLE, 50, 200, pos2, or2, 10,
        10000, 1000, 0, 1, zeros7);
...
TEST(opticalElements, ImagePlane) {
    ...
    std::shared_ptr<RAYX::PlaneMirror> plM =
        std::make_shared<RAYX::PlaneMirror>(
            ...
            RAYX::Geometry::GeometricalShape::RECTANGLE, 50, 200, pos, or1,
...
TEST(dynamicTracing, FourMirrors_9Rays) {
    ...
    std::shared_ptr<RAYX::PlaneMirror> p1 = std::make_shared<RAYX::PlaneMirror>(
        ...
        50, 200, pos, or1, zeros7);  // {1,2,3,0.01,0.02,0.03}
    ...
    std::shared_ptr<RAYX::PlaneMirror> p2 = std::make_shared<RAYX::PlaneMirror>(
        ...
        pos2, or2, zeros7);  // {1,2,3,0.01,0.02,0.03}
    ...
    std::shared_ptr<RAYX::PlaneMirror> p3 = std::make_shared<RAYX::PlaneMirror>(
        ...
        pos3, or3, zeros7);  // {1,2,3,0.01,0.02,0.03}
    ...
    std::shared_ptr<RAYX::PlaneMirror> p4 = std::make_shared<RAYX::PlaneMirror>(
        ...
        pos4, or4, zeros7);  // {1,2,3,0.01,0.02,0.03}
...
TEST(opticalElements, toroid) {
    ...
    std::shared_ptr<RAYX::ToroidMirror> t =
        std::make_shared<RAYX::ToroidMirror>(
            ...
            t_position, t_orientation, degToRad(10), 10000, 1000, 10000, 1000,
            zeros7);
    std::shared_ptr<RAYX::ImagePlane> ip = std::make_shared<RAYX::ImagePlane>(
        "Image plane", ip_position, ip_orientation);
...
TEST(PeteRZP, spec1_first_rzp) {
    ...
    std::shared_ptr<RAYX::ReflectionZonePlate> rzp =
        std::make_shared<RAYX::ReflectionZonePlate>(
            ...
            0, 4, 60, position, orientation, p->getPhotonEnergy(), -1, -1, 2.2,
            1, 90, 400, 90, 400, 0, 0, 1, 0,
...
TEST(PeteRZP, spec1_first_ip) {
    ...
    std::shared_ptr<RAYX::ReflectionZonePlate> rzp =
        std::make_shared<RAYX::ReflectionZonePlate>(
            "ReflectionZonePete", RAYX::Geometry::GeometricalShape::RECTANGLE,
            0, 4, 60, rzp_position, rzp_orientation, p->getPhotonEnergy(), -1,
            -1, 2.2, 1, 90, 400, 90, 400, 0, 0, 1, 0,
            zeros7);  // dx,dy,dz, dpsi,dphi,dchi //
...
TEST(PeteRZP, spec1_first_plus_rzp) {
    ...
    std::shared_ptr<RAYX::ReflectionZonePlate> rzp =
        std::make_shared<RAYX::ReflectionZonePlate>(
            "ReflectionZonePete", RAYX::Geometry::GeometricalShape::RECTANGLE,
            0, 4, 60, rzp_position, rzp_orientation, p->getPhotonEnergy(), 1, 1,
            2.2, 4.75, 90, 400, 90, 400, 0, 0, 1, -24.35,
            zeros7);  // dx,dy,dz, dpsi,dphi,dchi //
