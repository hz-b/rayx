...
TEST(Quadric, testGlobalCoordinates) {
    ...
    RAYX::WorldCoordinates w_coord = RAYX::WorldCoordinates(degToRad(10), degToRad(10), degToRad(7), 2000, std::vector<double>{0.5,0.7,1.0, 0.04,0.03,0.05});
    glm::dvec4 pos = w_coord.calcPosition();
    glm::dmat4x4 or1 =  w_coord.calcOrientation();
    std::shared_ptr<RAYX::PlaneMirror> p1 = std::make_shared<RAYX::PlaneMirror>("PlaneMirror1", 50, 200, 10, 7, 2000, std::vector<double>{0.5,0.7,1.0, 0.04,0.03,0.05}, sE, nullptr, true); // {1,2,3,0.01,0.02,0.03}
    ...
    RAYX::WorldCoordinates w_coord2 = RAYX::WorldCoordinates(degToRad(15), degToRad(15), degToRad(4), 7000, std::vector<double>{2,4,6, 0.04,0.01,0.06});//std::vector<double>{0,0,0, 0,0,0});
    glm::dvec4 pos2 = w_coord2.calcPosition(w_coord, pos, or1);
    glm::dmat4x4 or2 =  w_coord2.calcOrientation(w_coord, pos, or1);
    std::shared_ptr<RAYX::PlaneMirror> p2 = std::make_shared<RAYX::PlaneMirror>("PlaneMirror2", 50, 200, 15, 4, 7000, std::vector<double>{2,4,6, 0.04,0.01,0.06}, sE, p1, true); // {1,2,3,0.01,0.02,0.03}
    ...
    std::shared_ptr<RAYX::PlaneMirror> p3 = std::make_shared<RAYX::PlaneMirror>("PlaneMirror3", 50, 200, 7, 10, 8000, std::vector<double>{4,5,3, 0.01,0.02,0.03}, sE, p2, true); // {1,2,3,0.01,0.02,0.03}
    ...
    std::shared_ptr<RAYX::PlaneMirror> p4 = std::make_shared<RAYX::PlaneMirror>("PlaneMirror4", 50, 200, 22, 17, 1000, std::vector<double>{10,3,2, 0.01,0.03,0.06}, sE, p3, true); // {1,2,3,0.01,0.02,0.03}
